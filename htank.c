...
double* temp = (double*)malloc(recive_size*sizeof(double));
    ...
    {
	int offset = rank * size; //some kind of size
        int recived_count=0;
        while(recived_count<16){
            MPI_Recv(temp, recive_size, MPI_DOUBLE, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD,&Stat); //Is this where sendbuf is being returned to?
	...
	//here i am trying to put the "final grid" together before sending it to pgrid
	if(int i= 1; i < size+1;i++){
	    if(int k = 1; j < size+1;j++){
	    	final_grid[getIndex(i,j)] = temp[getIndex(i,j)] ;	
	    }
	}
        printf("\n");
        printf("\n");
...
double* sendbuf = (double*)malloc(sendsz*sizeof(double));
    ...
    for(double timestep=0; timestep<T; timestep+=dt)
    {
            ...
            flipMat_d(next, size+2);
            //prep for send
            int offset = rank * size; //to get the value in the grid based on the rank
            int t= offset; //used to point the send buf to the offset
            for(int i=1;i<size+1;i++) //here not sure if i should be changing the bounds that are being sent through the loops 
            {
                for(int j=1;j<size+1;j++)
                {
                sendbuf[t] = grid_ptr[i][j]; // this info is being sent to the send buff
                t++;
            }}
            //send to printer
